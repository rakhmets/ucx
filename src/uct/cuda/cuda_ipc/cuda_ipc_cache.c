...
uct_cuda_ipc_open_memhandle(uct_cuda_ipc_rkey_t *key, CUdevice cu_dev,
                            ucs_log_level_t log_level, CUdeviceptr *mapped_addr)
{
    ...
    case UCT_CUDA_IPC_KEY_HANDLE_TYPE_LEGACY:
        return uct_cuda_ipc_open_memhandle_legacy(key->ph.handle.legacy, cu_dev,
                                                  log_level, mapped_addr);
#if HAVE_CUDA_FABRIC
    case UCT_CUDA_IPC_KEY_HANDLE_TYPE_VMM:
        return uct_cuda_ipc_open_memhandle_vmm(key, cu_dev, log_level,
                                               mapped_addr);
    case UCT_CUDA_IPC_KEY_HANDLE_TYPE_MEMPOOL:
        return uct_cuda_ipc_open_memhandle_mempool(key, cu_dev, log_level,
                                                   mapped_addr);
#endif
...
UCS_PROFILE_FUNC(ucs_status_t, uct_cuda_ipc_map_memhandle,
                 (key, cu_dev, log_level, mapped_addr),
                 uct_cuda_ipc_rkey_t *key, CUdevice cu_dev,
                 ucs_log_level_t log_level, void **mapped_addr)
{
    uct_cuda_ipc_cache_t *cache;
    ucs_status_t status;
    ucs_pgt_region_t *pgt_region;
    ...
    status = uct_cuda_ipc_open_memhandle(key, cu_dev, log_level,
                                         (CUdeviceptr*)mapped_addr);
        ...
        if (ucs_likely(status == UCS_ERR_ALREADY_EXISTS)) {
            ...
            if (ucs_unlikely(status != UCS_OK)) {
                if (ucs_likely(status == UCS_ERR_ALREADY_EXISTS)) {
                    ...
                    status =
                        uct_cuda_ipc_open_memhandle(key, cu_dev, log_level,
                                                    (CUdeviceptr*)mapped_addr);
                    if (status != UCS_OK) {
